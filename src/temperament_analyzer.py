...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('temperament_analyzer.log'),
...
class MusicLibraryClient(ABC):
    ...
    @abstractmethod
    def get_playlists(self, metadata_client: TrackMetadataClient = None) -> List[Playlist]:
...
class MusicAppClient(MusicLibraryClient):
    ...
    def get_playlists(self, metadata_client: TrackMetadataClient = None) -> List[Playlist]:
        ...
        try:
            ...
            if self.whitelist_enabled and self.whitelist:
                playlists_to_process = {name: pid for name, pid in playlists_with_ids.items() if name in self.whitelist}
                logger.info(f"Whitelist enabled: Filtered to {len(playlists_to_process)} whitelisted playlists")
            else:
    ...
    def _get_playlist_with_tracks_by_id(self, playlist_name: str, playlist_id: str, metadata_client: TrackMetadataClient = None) -> Optional[Playlist]:
...
tell application "Music"
    ...
    def _run_applescript_file(self, script_path: str, args: List[str] = None) -> str:
...
class OpenAILLMClient(LLMClient):
    ...
    def classify_track(self, track: Track) -> ClassificationResult:
        ...
        try:
            ...
            logger.debug(f"Track '{track.name}' classified as {temperament.value} (confidence: {classification.get('confidence', 0.5):.2f})")
        ...
        try:
            ...
            payload = {
                ...
                'messages': [
                    {
                        'role': 'system',
                        'content': SYSTEM_PROMPT_PLAYLIST
                    },
                    {
                        'role': 'user',
                        'content': prompt
            ...
            logger.debug(f"Playlist '{playlist.name}' classified as {temperament.value} (confidence: {classification.get('confidence', 0.5):.2f})")
            ...
            dominant_temp = temperament_counts.most_common(1)[0][0] if temperament_counts else Temperament.FROLIC
            return ClassificationResult(
...
class TemperamentAnalyzer:
    ...
    def analyze_and_organize(self, playlists: Optional[List[Playlist]] = None, batch_size: int = 10) -> bool:
        ...
        for idx, playlist in enumerate(playlists, 1):
            logger.info(f"Processing playlist {idx}/{len(playlists)}: {playlist.name} (ID: {playlist.playlist_id})")
            ...
            try:
                ...
                for track_idx, track in enumerate(playlist.tracks, 1):
                    ...
                    if cache_key in self.track_cache:
                        ...
                        logger.debug(f"Classified {track_idx}/{len(playlist.tracks)}: {track.name} -> {classification.temperament.value}")
                ...
                playlist_classification = self.llm_client.classify_playlist(playlist, track_classifications)
                
                logger.info(f"Playlist '{playlist.name}' classified as: {playlist_classification.temperament.value} "
