...
class PlaylistClassifier:
        ...
        for file_path in artist_dir.glob("*.json"):
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    if 'artists' in data:
                        data['artists'] = [self.normalizer.normalize(artist) for artist in data['artists']]
                    if 'keywords' in data:
                        data['keywords'] = [self.normalizer.normalize(kw) for kw in data['keywords']]
                    artist_lists[genre] = data
                    logger.debug(f"Loaded {len(data.get('artists', []))} artists for genre: {genre}")
            except Exception as e:
    ...
    def map_genre_to_target(self, raw_genre: str) -> Optional[str]:
        ...
        genre_keywords = {
            'hiphop': ['hip hop', 'rap', 'trap', 'drill', 'gangsta rap', 'conscious rap', 'old school'],
            'electronic': ['electronic', 'edm', 'techno', 'house', 'trance', 'dubstep', 'ambient', 
                         'electro', 'synth', 'dance', 'dnb', 'drum and bass', 'hardcore'],
            'disco_funk_soul': ['disco', 'funk', 'soul', 'motown', 'r&b', 'groove', 'boogie'],
            'jazz': ['jazz', 'bebop', 'fusion', 'cool jazz', 'smooth jazz', 'avant-garde', 'swing'],
            'world': ['world', 'folk', 'traditional', 'ethnic', 'african', 'latin', 'reggae', 
                     'caribbean', 'middle eastern', 'asian', 'indian'],
            'rock': ['rock', 'metal', 'punk', 'indie', 'alternative', 'grunge', 'hard rock', 
                    'progressive', 'classic rock', 'pop rock']
        }
    ...
    def score_playlist(self, tracks: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for i, track in enumerate(tracks):
            ...
            if logger.isEnabledFor(10):  # DEBUG level is 10
                max_genre = max(track_scores.items(), key=lambda x: x[1]) if track_scores else ('none', 0)
                logger.debug(f"Track {i+1}: {track.get('artist', 'Unknown')} - "
    ...
    def determine_dominant_genre(self, playlist_scores: Dict[str, float], 
                               tracks: List[Dict[str, Any]], 
                               playlist_name: str = "") -> Tuple[Optional[str], Dict[str, Any]]:
        """
        ...
        if dominance_ratio >= self.dominance_threshold:
            ...
            decision_info['reason'] = f'No clear dominance (max: {dominance_ratio:.2f}, threshold: {self.dominance_threshold})'
            return None, decision_info

    def classify_playlist(self, tracks: List[Dict[str, Any]], 
                         playlist_name: str = "") -> Tuple[Optional[str], Dict[str, Any]]:
        """
        ...
        logger.info(f"Classification result for '{playlist_name}': "
